...
function ok(data: unknown, extra: ContentBlock[] = []): ToolResult {
	...
	if (data && typeof data === "object") {
		res.structuredContent = Array.isArray(data)
			? { items: data }
...
export function registerAll(server: McpServer, api: SoundCloudAPI): void {
	...
	server.registerTool(
		...
		async () =>
			run(async () => {
				if (!hasUserToken())
					return "Not logged in. Use connect_soundcloud (public search still works).";
	...
	server.registerTool(
		...
		async ({ limit, nextPage }) =>
			run(() => (nextPage ? api.getNextPage(nextPage) : api.getMyLikes(limit)))
	);
	...
	server.registerTool(
		...
		async ({ playlistId, ...updates }) =>
			run(() => api.updatePlaylist(playlistId, updates), playlistLinks)
	);
...
function registerPrompts(server: McpServer, api: SoundCloudAPI): void {
	server.registerPrompt(
		"analyze_music_taste",
		{
			...
			description: "Analyze a user's music taste based on their liked tracks",
		},
		async () => {
...
function registerResources(server: McpServer, api: SoundCloudAPI): void {
	...
	server.registerResource(
		...
		{
			title: "My playlists",
			description: "The logged-in user's playlists",
			mimeType: "application/json",
		},
		async (uri) => ({
			contents: [
	...
	server.registerResource(
		...
		{
			title: "My likes",
			description: "The logged-in user's liked tracks",
			mimeType: "application/json",
		},
		async (uri) => ({
			contents: [
